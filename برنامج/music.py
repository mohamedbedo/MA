...
@Client.on_message(command(["mplay", f"mplay@{BOT_USERNAME}", "play", "تشغيل ", "اتكلم", "شغل"]) & other_filters)
    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
             ...
             try:
                await suhu.edit("🔎 **انتظر يتم البحث...**")
            ...
            if len(m.command) < 2:
                await m.reply(
                    "» رد علي **ملف صوتي** او **إعطاء شيء للبحث.**"
                )
            else:
                suhu = await c.send_message(chat_id, "🔍 **انتظر جاري البحث...**")
